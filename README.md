...
- Copy-shadow template-disjoint challenge v1: `CHALLENGE_EVALUATION_BLOCKED`; 120 public-safe rows are frozen with 30 rows per enabled scope and 30 disabled action controls; template-disjoint audit passes, but observed challenge inference did not run because no local loadable frozen adapters were identity-verifiable.
- Current blocked next step: resolve frozen adapter identity, then rerun this same frozen challenge through the canonical prediction hook; still observe-only and no runtime enforcement.

No model weights changed during the Contract V2 projection, slot mechanism analysis, hybrid slot representation design, copy-backed verification slice, shadow-mode integration, shadow interface review, prediction shadow hook, or template-disjoint challenge freeze. strict exact remains canonical diagnostic. Prior metrics are historical unless marked `CURRENT` in the evidence index.
...
| [`reports/public-sample/copy-shadow-template-disjoint-challenge-v1/challenge-summary.json`](reports/public-sample/copy-shadow-template-disjoint-challenge-v1/challenge-summary.json) | Template-disjoint challenge v1 result: `CHALLENGE_EVALUATION_BLOCKED`; 120 rows frozen, adapter-backed observed inference blocked. |
