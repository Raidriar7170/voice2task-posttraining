...
当前结论很保守：7B LoRA 路径已经能在 A100 私有运行环境中跑通。最新 current evidence 是 Contract V2 projection blocked evidence：step-matched canonical-slot SFT ablation 的 Control / Treatment 使用相同 optimizer-step budget，结果 mixed / inconclusive；随后离线 Contract V2 projection 因缺少当前 raw prediction / gold contract artifacts 而 fail closed。当前不能继续小批 canonical candidate loop，不能 DPO，也不能宣称 Contract V2 值得实施；下一步只能先恢复或提交 public-safe 的当前 step-matched raw artifacts，再重跑同一个 bounded projection evaluation。
...
- 最新公开证据：[`contract-v2-projection`](reports/public-sample/contract-v2-projection/decision.md) 是当前 blocked projection evidence；其输入边界来自 [`step-matched-canonical-slot-ablation`](reports/public-sample/step-matched-canonical-slot-ablation/decision.md)。
