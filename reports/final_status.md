...
当前最新推荐来自 `select-residual-driven-remediation-target`：

- 输入：`reports/public-sample/remediation-target-selection/summary.json`、
  `reports/public-sample/remediation-target-selection/top-failures.md` 和
  `reports/public-sample/remediation-target-selection/recommended-next-change.md`；
- 推荐下一阶段 change id：`design-safety-repair-candidates`；
- 目的：先为 unsafe false negative / safety boundary 设计 public-safe repair
  candidates 和 fail-closed safety examples，再考虑 slot canonicalization、
  data merge 或 training；
