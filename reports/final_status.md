...
| Latest model evidence pack | `reports/public-sample/a100-current-manifest-sft-v3-prediction-baseline/` |
...
| Latest run type | prediction-only eval with existing private SFT v3 adapter |
...
最新模型证据现在绑定 `public-sample-20260616T165835Z`。它复用已有 private SFT v3 adapter 做 dev/test prediction-only，没有训练、没有修 prediction、没有 normalize slot，也没有放松 evaluator。adapter、checkpoint、raw log、private override 和远端缓存都没有发布。旧 SFT v3 retry 仍是 adapter 来源和 prior boundary，但不再是当前 manifest 的最新评估。
...
由于 `public-sample-20260616T165835Z` 的新增 blocked-payment repair rows 只进入 train split，dev/test row ids 与 prior SFT v3 retry 保持一致，因此这次 current-manifest prediction-only 的指标与 prior retry 一致。它的价值是锁定当前 manifest 边界，而不是证明新增 repair rows 已被模型学习。

相对旧 prediction-only baseline（`public-sample-20260616T074315Z` 上的 prior adapter baseline）：dev exact `+0.1594`、dev strict slot_f1 `+0.1739`；test exact `+0.0580`、test strict slot_f1 `-0.0097`。这个比较跨越训练阶段但不是当前 blocked-payment materialization 的直接效果。dev safety_recall 仍为 `0.5556`，这是下一阶段必须优先处理的风险。
...
因此当前模型效果应以本报告上方的 current-manifest SFT v3 prediction-only 指标为准：它是局部改善，
...
- Latest evidence report: `reports/public-sample/a100-current-manifest-sft-v3-prediction-baseline/report.md`
- Latest Dev metrics: `reports/public-sample/a100-current-manifest-sft-v3-prediction-baseline/dev/metrics.md`
- Latest Test metrics: `reports/public-sample/a100-current-manifest-sft-v3-prediction-baseline/test/metrics.md`
