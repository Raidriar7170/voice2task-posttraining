...
- Copy-shadow false-trust diagnosis: `SOURCE_ATTESTATION_HARDENED_SCOPE_REDUCTION_REQUIRED`; historical source-attested exact input count is 64, source-attested-but-gold-mismatch count is 16, normalized-equivalent collision downgrades are 6, post-diagnosis source_attested_exact count is 58, and execution eligible count is 0.
- Current boundary: `trusted_provenance` is only a deprecated compatibility alias; new artifacts use source-only `source_attested_exact`. Challenge v1 is observe-only verifier-fixture evidence with a failed safety gate; no runtime enforcement, action trusted provenance, normalized trusted provenance, model-quality claim, or safety/production readiness claim follows from it.
...
| [`reports/public-sample/copy-shadow-false-trust-diagnosis/summary.json`](reports/public-sample/copy-shadow-false-trust-diagnosis/summary.json) | False-trust diagnosis result: `SOURCE_ATTESTATION_HARDENED_SCOPE_REDUCTION_REQUIRED`; form field proposes disable, search query limited observe, extract target observe-only. |
...
The Contract V2 projection is offline schema-burden evidence only: it removes derived/display-field burden from strict exact comparison, but it does not claim model improvement. The internal Contract V2 Core boundary now exists behind a V1-compatible deterministic envelope; it does not change the public V1 schema, V1 evaluator, training target, predictions, or downstream runtime. The slot mechanism analysis and hybrid slot representation design are read-only/design-only evidence. The copy-backed verification slice, shadow-mode integration, shadow interface review, prediction shadow hook, recovered-adapter challenge run, and false-trust diagnosis are provenance/interface evidence only. Challenge v1 is observed through recovered frozen adapters, but it remains a verifier adversarial fixture, not naturalistic generalization or task correctness evidence. Source-backed provenance is not task correctness, slot accuracy, executable quality, runtime enforcement, production readiness, or challenge-set generalization. The only recommended next copy-shadow change is `design-copy-shadow-scope-policy-v2`; do not create policy v2, naturalistic v2, training, action enablement, or normalized trusted provenance from the current diagnosis.
